...
class SetWorldSpawnCommand extends VanillaCommand {
	public function __construct($name){
		parent::__construct("setworldspawn", "Устанавливает точку респауна мира. Если координаты не введены, будут использованы координаты игрока");
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
		...
		if(count($args) === 0){
			if($sender instanceof Player){
				...
				$sender->sendMessage(TextFormat::RED . "Данную команду нельзя исполнить из консоли");
				return true;
			}
		}elseif(count($args) === 3){
