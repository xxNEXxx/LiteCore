...
class TeleportCommand extends VanillaCommand {
	public function __construct($name){
		parent::__construct("tp", "Телепортирует определённого игрока (или Вас) к другому игроку или на другие координаты");
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
		...
		if(count($args) === 1 or count($args) === 3 or count($args) === 5){
			...
			if(count($args) === 1){
				...
				if($target === null){
					$sender->sendMessage(TextFormat::RED . "Невозможно найти игрока " . $args[0]);
					return true;
				}
			}
		}else{
			...
			if(count($args) === 2){
				...
				if($target === null){
					$sender->sendMessage(TextFormat::RED . "Невозможно найти игрока " . $args[1]);
					return true;
				}
			}
		}
